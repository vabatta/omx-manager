 ...
 * @throws {TypeError} Invalid parameters.
 * @throws {Error} Could not execute command correctly.
 */
OmxManager.prototype.setOmxCommand = function setOmxCommand(cmd) {
  ...
  else if (is.not.equal(ChildProcess.spawnSync('which', [cmd]).status, 0)) throw new Error('Error: cmd: cannot execute command correctly.');
